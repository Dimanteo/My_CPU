...
/* DEF_CMD(CMD_name, token, scanf_sample, number_of_args, instructions, disasm_print)*/
    ...
    DEF_CMD(SUB, sub, /*scanf_sample*/, 0, int x = cPOP; cPUSH(cPOP - x), /*disasm_print*/)
    ...
    DEF_CMD(POPRAM_NX, push, SCANsample(" [%[^]]]", sarg), 2, cpu.RAM[arg_v[0] + cREG(arg_v[1]) / PRECISION] = cPOP, " [%d + %s]" $ *(int*)(pc + 1) $ translate_code(*(int*)(pc + 1) + 1))
    DEF_CMD(POPRAM_XN, push, SCANsample(" [%[^]]]", sarg), 2, cpu.RAM[arg_v[1] + cREG(arg_v[0]) / PRECISION] = cPOP, " [%s + %d]" $ translate_code(*(int*)(pc + 1)) $ *((int*)(pc + 1) + 1))
    DEF_CMD(POPRAM, push, SCANsample(" [%[^]]]", sarg), 1, cpu.RAM[arg_v[0]] = cPOP, " [%d]" $ *(int*)(pc + 1))
    DEF_CMD(PUSHRAM, pop, SCANsample(" [%[^]]]", sarg), 1, cPUSH(cpu.RAM[arg_v[0]]), " [%d]" $ *(int*)(pc + 1))
    DEF_CMD(POPRAM_X, push, SCANsample(" [%[^]]]", sarg), 1, cpu.RAM[cREG(arg_v[0]) / PRECISION] = cPOP, getARG_REG)
    DEF_CMD(PUSHRAM_X, pop, SCANsample(" [%[^]]]", sarg), 1, cPUSH(cpu.RAM[cREG(arg_v[0]) / PRECISION]), getARG_REG)
    DEF_CMD(PUSHRAM_NX, pop, SCANsample(" [%[^]]]", sarg), 2, cPUSH(cpu.RAM[arg_v[0] + cREG(arg_v[1]) / PRECISION]), " [%d + %s]" $ *(int*)(pc + 1) $ translate_code(*(int*)(pc + 1) + 1))
    DEF_CMD(PUSHRAM_XN, pop, SCANsample(" [%[^]]]", sarg), 2, cPUSH(cpu.RAM[arg_v[1] + cREG(arg_v[0]) / PRECISION]), " [%s + %d]" $ translate_code(*(int*)(pc + 1)) $ *((int*)(pc + 1) + 1))
